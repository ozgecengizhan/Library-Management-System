...
def Edit_Book_Info(self):
        ...
        while True:
            ...
            if book_to_edit.strip() == "b" or book_to_edit == "5":
                print(" \n Back to the menu...")
                return
            elif not book_to_edit.strip():  # Check if title_to_remove is empty or contains only whitespace
                print(" Invalid input! Please enter a valid book to edit.")
                continue  # Tekrar sormak için continue kullanıyoruz
            ...
            # Hangi bilgiyi düzenlemek istediğini sor
            ...
            if choice == "1":
                ...
                field_name = "Release Year"
            elif choice == "4":
                field_name = "Number of Pages"
            elif choice == "5":
                field_name = "Back to the menu"
                return
            else:
                print("Invalid choice.")
                continue  # Tekrar sormak için continue kullanıyoruz

            # Yeni bilgiyi al ve güncelle
            while True:
                ...
                if not new_info:  
                    ...
                    continue  # Tekrar sormak için continue kullanıyoruz
                elif new_info == "b" or new_info == "5":
                    print("\nBack to the menu...")
                    return
                elif choice == "2" and new_info.isdigit():
                    print("Invalid input! Please enter a valid value (cannot be a number).")
                    continue  # Tekrar sormak için continue kullanıyoruz
                elif choice == "3" and not (new_info.isdigit() and len(new_info) == 4 and 1 <= int(new_info) <= 9999):
                    print("Invalid input! Release year must be a numeric value between 1-9999 in 'YYYY' format.")
                    continue  # Tekrar sormak için continue kullanıyoruz
                elif choice == "4" and (not new_info.isdigit() or int(new_info) <= 0):
                    print("Invalid input! Please enter a valid value (must be a positive integer).")
                    continue  # Tekrar sormak için continue kullanıyoruz
                else:
                    break
            ...
            with open("books.txt", "r+", encoding="utf-8") as file:
                lines = file.readlines()
                lines[book_index] = lines[book_index].replace(lines[book_index].split(',')[['Book Title', 'Author', 'Release Year', 'Number of Pages', 'Back to the menu'].index(field_name)], new_info)
                file.seek(0)
